# Hospital Management System — Desktop (v1.0)

A local JavaFX desktop application for managing hospital administration and related care records, backed by SQLite.

This directory holds the complete **v1.0 desktop application**: its Maven build (`pom.xml`), Java sources and JavaFX resources (`src/`), automated tests, and its local `data/` directory. It is the `desktop/` folder of the [Hospital Management System monorepo](../README.md), which also reserves `web/` for the planned v2.0 web application (not implemented yet). All Maven commands in this document are run from `desktop/`.
...
```text
desktop/                          # v1.0 JavaFX desktop application (this directory)
├── README.md                     # This document
├── pom.xml                       # Maven build (Java 21, JavaFX 21, SQLite, JUnit 5)
├── data/
...
Paths above are relative to this `desktop/` directory. The repository root holds the
shared [`.gitignore`](../.gitignore), the monorepo [`README.md`](../README.md), and the
[`docs/`](../docs/) directory used by this project.
...
Then test and launch **from this `desktop/` directory**:
...
Run Maven from `desktop/` (not the repository root) so the relative database location is consistent. The first launch initializes the SQLite schema and seeds demo accounts if the users table is empty. No manual schema import is required.
...
Run the automated suite from this `desktop/` directory (`cd Hospital-Management-System/desktop` from a fresh clone):
...
Database tests use isolated temporary SQLite files through `TestDatabaseFactory`, rather than the application's own database at `desktop/data/hospital.db`. Maven writes test reports to `desktop/target/surefire-reports/` (`target/surefire-reports/` inside this directory) after execution.
...
**Monorepo reorganization:** the v1.0 project was moved from the repository root into `desktop/` unchanged. Only this document, the repository-root `.gitignore` database paths, and the directory layout were touched; no Java source, test, resource, Maven dependency, Java 21 configuration, or database schema changes were made. Maven was not available in the reorganization sandbox either, so `mvn clean test` could not be executed there; run it locally from `desktop/` to confirm the suite.
...
- **Storage:** SQLite file at `desktop/data/hospital.db`, relative to the working directory where the application is launched. Launch Maven from `desktop/` and the file is created inside this directory's `data/` folder.
...
The database and its SQLite journal/WAL sidecar files are excluded from Git by the repository-root `.gitignore`; only `desktop/data/.gitkeep` is tracked. Do not commit databases containing personal data. Close the application before making a filesystem backup of the database, and keep any backups outside the repository. Starting from another working directory (for example the repository root) can create a different `data/hospital.db` there.
...
When real desktop captures are available, add them under a dedicated directory in the repository-root `docs/` folder (`../docs/`) and link the actual committed files here. Useful views include Login, Dashboard, a table/form workspace, Billing, Reports, and a styled dialog. Use synthetic data and omit any personal information. No screenshot image links are included until those files exist.
...
- Before submitting changes, run `mvn clean test` from `desktop/` and `git diff --check`; exercise affected desktop workflows as well.
- Do not commit `target/`, compiled classes, IDE settings, logs, local database files, SQLite sidecars, or private environment files. The repository-root `.gitignore` covers these generated/local files — under both the root and `desktop/` — without ignoring application source or tests.
